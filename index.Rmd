...
First, [download the Geoscripting OVA image by following this link](https://wageningenur4-my.sharepoint.com/:u:/g/personal/dainius_masiliunas_wur_nl/EUk9OnWEtiJGuUIShYY6VvgBgVlKpInzibv_DxxBnnIRbQ?e=dYihYs).
...
If you couldn't get a VirtualBox image running due to hardware limitations, below are several alternative methods of getting Ubuntu with Geoscripting software working. In all cases, please see the last section on how to install the needed software.
...
The VirtualBox OVA image above is made for Intel architectures. Other architectures, like ARM that is used on some newer macOS devices, are not supported.
...
Unfortunately there is no desktop version of Ubuntu 24.04 for ARM, but there is a server version, which will also work.
Download the [Ubuntu 24.04 ARM ISO from here](https://ubuntu.com/download/server/arm).
...
Press *New*, give it a name, put the ISO image you just downloaded into the *ISO Image* box, select the 24.04 version in the "Version" box, and check the box "skip unattended install".
...
You navigate with arrow keys, Tab and Enter.
You can accept all defaults, except that you should **disable LVM** when prompted, as that would only let you use half of your allocated disk space.
When the installation is finished, press *Reboot Now*.
When prompted to remove the installation medium, press Enter.

After it reboots, press Enter, log in and install the package `ubuntu-desktop` by entering the following code:
...
Finally, reboot using the command `systemctl reboot` and continue with installing the Geoscripting software by following the [last section of this page](#installing-necessary-software).
