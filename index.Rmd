...
First, [download the Geoscripting image by following this link](https://wageningenur4-my.sharepoint.com/:u:/g/personal/dainius_masiliunas_wur_nl/EUnuNnIfUlxJtjm9N-qFV6cB8fc64d7iC3PT2QulfN_wFA?e=vLk3hA&isSPOFile=1).
...
We will use this environment throughout the course, so explore it and get familiar with it. You can press the Ubuntu logo 9circle with three dots) at the bottom left to see all the software isntalled on the image.
...
It will ask for your user's password, which is `osboxes.org`. You will not see the password when you type it (it's a security feature, but it remembers what you typed). After that, type `Y` to confirm removal of the old Guest Additions. Now that it's finished, you can go back to the CD image directory and again click on "Run Software". This time, it should install succesfully. After that, restart the Virtual Machine, and your screen should resize successfully!
