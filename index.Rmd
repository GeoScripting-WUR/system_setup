---
pagetitle: "Setting up your system for Geoscripting with R"
author: "Dainius Masiliunas, Loïc Dutrieux, Jan Verbesselt"
...
If you don't manage, you can go to the [Servicepoint IT](https://wur.service-now.com/esc/en/laptop-software-service/laptop-software-service-for-students?id=kb_article&sysparm_article=KB0017150) (see under the heading "Who can help me if I have issues with my laptop?") and ask the staff there to help you set up VirtualBox on your computer.
...
First, [download the Geoscripting OVA image by following this link](https://wageningenur4-my.sharepoint.com/:u:/g/personal/dainius_masiliunas_wur_nl/IQBk2nnF36u4QLX7avq9_4lbAVJz2MKT8AN0ThqgfIBB6I0?e=Ounwcl).
...
Too many cores increases the chances of lock-ups.
...
The username to use is `osboxes` and the password is `osboxes.org`.
This is also the administrator password.
You can change it in the settings if you wish.

If you got this far, good job, you are now running Linux!
We will use this environment throughout the course, so explore it and get familiar with it. You can press the Ubuntu logo (circle with three dots) at the bottom left to see all the software installed on the image.

When it comes time to shut down your environment, click the power button at top right, then again click the power button, and click *power off*. **Don't use the *X* button of the VirtualBox window in your host OS, unless it is frozen.**
...
If you couldn't get a VirtualBox image running due to hardware limitations, below are several alternative methods of getting Ubuntu with Geoscripting software working. In all cases, **please also see the last section on how to install the needed software.**
...
## For non-Intel macOS and other ARM users

The VirtualBox OVA image above is made for Intel/amd64 architectures. Other architectures, like ARM that is used on some newer macOS devices, are not supported.
Please purchase a device that can run Intel architecture software, as you will continue to run into these issues in future courses as well.

If you have no other choice (e.g. you don't have enough time to buy a new laptop), a workaround for this course only is to install an ARM version of VirtualBox and install the ARM version of Ubuntu into it.
Download the [Ubuntu 26.04 ARM ISO from here](https://ubuntu.com/download/desktop).
<!--In VirtualBox, you can try to automatically set it up by pressing *New*, giving it a name, putting the ISO image you just downloaded into the *ISO Image* box, selecting the 24.04 version in the "Version" box, and filling in the information in the "Unattended Install" section.
This will install everything on first launch.
If that doesn't work, try to add it again, -->
Press *New*, give it a name, put the ISO image you just downloaded into the *ISO Image* box, select the 26.04 version in the "Version" box, and uncheck the box "Proceed with Unattended Installation".
In the *Hard Disk* tab, **make sure to give your virtual machine at least 50 GB space**, ideally set it to 200-500 GB.
This is the maximum the disk space can grow to, but it will not take as much space, at least initially.
Start the virtual machine, go through the installation wizard.
You can accept all defaults.
When the installation is finished, press *Reboot Now*.
When prompted to remove the installation medium, press Enter.

Finally, continue with installing the Geoscripting software by following the [last section of this page](#installing-necessary-software).
Ignore any errors that appear (some packages are not available on ARM, but they are not critical).

## Linux on a USB drive

Download the ISO image of [Ubuntu 26.04](http://releases.ubuntu.com/26.04/), and write it onto a USB stick. There are several programs to do so depending on your operating system, e.g. [Rufus](https://tutorials.ubuntu.com/tutorial/tutorial-create-a-usb-stick-on-windows) on Windows.
...
This is not required if you use the pre-made VirtualBox image above, but is required in all other cases.
