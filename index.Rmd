...
The VirtualBox ISO image above is made for Intel architectures. Other architectures, like ARM that is used on some newer macOS devices, are not supported.
...
If that doesn't work, try to add it again, check the box "skip unattended install" and follow the instructions for [manually installing Ubuntu on VirtualBox](https://itslinuxguide.com/install-ubuntu-virtualbox/).
Eiter way, **make sure to give your virtual machine at least 50 GB space**.

When the installation is finished, log in and install the package `ubuntu-desktop` using the following code:
